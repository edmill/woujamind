...
export function ResultView({
  ...
  // Close art style dropdown when clicking outside
  useEffect(() => {
                  ...
                  <div className="flex items-center gap-1">
                     ...
                     <div className="relative" ref={artStyleDropdownRef}>
                       <button
                         ...
                         disabled={isEditing}
                         className="flex items-center gap-1.5 px-2 py-1.5 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                         title="Change Art Style"
                       >
                         <div className={cn("w-4 h-4 rounded bg-gradient-to-r", currentArtStyle.previewColor)} />
                         <span className="hidden lg:inline text-xs font-semibold">{currentArtStyle.label}</span>
                         <ChevronDown className={cn("w-3 h-3 transition-transform", showArtStyleDropdown && "rotate-180")} />
                       </button>
                       ...
                       <AnimatePresence>
                         {showArtStyleDropdown && (
                           <motion.div
                             ...
                             exit={{ opacity: 0, y: -10 }}
                             className="absolute top-full left-0 mt-2 w-72 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl overflow-hidden z-50"
                           >
                             <div className="p-2">
                               ...
                               {ART_STYLES.map((style) => (
                                 ...
                                 >
                                   <div className={cn("w-10 h-10 rounded-lg bg-gradient-to-r flex-shrink-0", style.previewColor)} />
                                   <div className="flex-1 min-w-0">
                                     <div className="font-bold text-sm">{style.label}</div>
                                     <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{style.description}</div>
                                   </div>
                                   {selectedArtStyle === style.id && (
