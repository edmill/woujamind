...
export const BlobPreview = ({ 
  ...
  const currentExpression = isGenerating ? 'neutral' : (mode === 'expression' ? expression : 'neutral');
  ...
  const activeAnim = isGenerating ? 'generating' : (mode === 'action' ? action : 'idle');
    ...
    <div ref={containerRef} className={cn(
      ...
      {!isTransparent ? (
        <>
          ...
          />
          {/* Sprite Sheet Grid Layout Hint */}
          <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
             <div className="grid grid-cols-4 grid-rows-2 gap-4 w-full h-full p-6">
                {[...Array(8)].map((_, i) => (
                  <div key={i} className="border-2 border-dashed border-slate-500 rounded-lg" />
                ))}
             </div>
          </div>
      ...
      >
        ...
        {/* Particles for Cast/Attack/Generating */}
        {(activeAnim === 'cast' || isGenerating) && (
          <motion.div 
            animate={{ rotate: 360, scale: isGenerating ? [1, 2, 0.5, 1.5] : [1, 1.5, 1] }}
            transition={{ repeat: Infinity, duration: isGenerating ? 1 : 2 }}
            className={cn(
               "absolute -inset-4 border-2 border-dashed rounded-full",
               isGenerating ? "border-orange-500/50 dark:border-orange-400/50 opacity-70" : "border-yellow-400/50 dark:border-yellow-300/50"
            )}
          />
        )}
