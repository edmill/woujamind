...
Output strictly the description string. Do not include intro/outro text.`;
  ...
  try {
    ...
    logger.warn("Analysis failed, falling back to user description", e);
    return {
...
Return ONLY the JSON object above, no additional text.`;
  ...
  try {
    ...
    logger.warn("Style analysis failed, using defaults", e);
...
Return ONLY valid JSON, no additional text.`;
  ...
  try {
    ...
    logger.warn("Multi-view detection failed", e);
...
OUTPUT: A single, high-quality reference image suitable for video-based sprite generation.`;
    ...
    logger.debug('[generateReferenceImage] Generating reference image with prompt:', userPrompt);
    logger.debug('[generateReferenceImage] Using model:', modelId);
      ...
      if (candidate.content?.parts) {
        for (const part of candidate.content.parts) {
          if (part.inlineData?.data) {
            ...
            logger.debug('[generateReferenceImage] Successfully generated reference image');
            return imageBase64;
          }
        }
      }
    ...
    logger.error('[generateReferenceImage] Error:', error);
...
export const generateSpriteSheet = async (
  ...
  logger.debug(`Custom rules (first 100 chars):`, customRules?.substring(0, 100));
  try {
    const ai = await getClient();
...
OUTPUT: ${totalFrames} clean, consistent animation frames in a ${rows}x${cols} invisible grid layout.`;
    ...
    logger.group('Sprite Generation');
    logger.debug(`Grid: ${rows}x${cols}, AR: ${bestRatio}, Model: ${modelId}`);
    logger.debug(`Art Style: ${artStyleId} -> "${stylePrompt?.substring(0, 50)}..."`);
    logger.debug(`Action: ${actionId} -> ${action.label}`);
    logger.debug(`Character: "${characterDescription?.substring(0, 100)}..."`);
    logger.debug(`Has reference image: ${!!imageBase64}`);
    logger.debug(`Full prompt (first 300 chars):\n`, prompt.substring(0, 300));
    ...
    if (imageBase64) {
      ...
      logger.debug(`Reference image size: ${cleanBase64.length} chars`);
    }

    logger.debug(`Calling API with model: ${modelId}`);
    const response = await ai.models.generateContent({
      model: modelId,
...
export const batchEditFrames = async (
  ...
  logger.group('Batch Edit');
  logger.debug(`Number of frames: ${frameImages.length}`);
...
OUTPUT: Generate the modified sprite sheet grid with ALL frames edited according to the request.`;
    ...
    logger.debug('Sending batch edit request to API...');
    const ai = await getClient();
    ...
    if (response.candidates?.[0]?.finishReason && response.candidates[0].finishReason !== 'STOP') {
      logger.error('Response was blocked:', response.candidates[0].finishReason);
    ...
    logger.debug(`Successfully extracted ${editedFrames.length} edited frames`);
    logger.groupEnd();
    return editedFrames;

  } catch (error) {
    logger.groupEnd();
    logger.error('Batch edit failed:', error);
...
export const editSpriteSheet = async (
  ...
  logger.debug(`Edit prompt: ${editPrompt}`);
  logger.debug(`Model: ${modelId}`);
  logger.debug(`Image data length: ${imageBase64?.length || 0}`);

  try {
    const ai = await getClient();
    logger.debug('Got AI client successfully');
...
OUTPUT: Generate the modified sprite sheet with CLEAR, VISIBLE changes as requested.`;
    ...
    logger.debug(`Prompt: "${editPrompt}"`);
    logger.debug(`Model: ${modelId}`);
    logger.debug(`Full prompt (first 200 chars): ${fullPrompt.substring(0, 200)}`);
    ...
    // Check for blocked or filtered responses
    if (response.candidates?.[0]?.finishReason && response.candidates[0].finishReason !== 'STOP') {
      logger.error('Response was blocked or filtered:', response.candidates[0].finishReason);
    ...
    if (response.candidates && response.candidates.length > 0) {
      for (const candidate of response.candidates) {
        if (candidate.content?.parts) {
          for (const part of candidate.content.parts) {
            if (part.inlineData?.data) {
              logger.debug('Successfully extracted edited image');
              logger.groupEnd();
...
export const generateInBetweenFrame = async (
  ...
  logger.group('In-Between Frame Generation');
  logger.debug(`Action: ${action}`);
  logger.debug(`Art style: ${artStyle}`);
  logger.debug(`Has before frame: ${!!frameBefore}`);
  logger.debug(`Has after frame: ${!!frameAfter}`);
...
OUTPUT: A single sprite frame that naturally precedes the reference frame.`;
    ...
    // Check for blocked or filtered responses
    if (response.candidates?.[0]?.finishReason && response.candidates[0].finishReason !== 'STOP') {
      logger.error('Response blocked:', response.candidates[0].finishReason);
    ...
    if (response.candidates && response.candidates.length > 0) {
      for (const candidate of response.candidates) {
        if (candidate.content?.parts) {
          for (const part of candidate.content.parts) {
            if (part.inlineData?.data) {
              logger.debug('Successfully generated in-between frame');
              logger.groupEnd();
              return `data:image/png;base64,${part.inlineData.data}`;
            }
          }
